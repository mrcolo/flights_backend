...
//            computeDijkstra(source, previous);
            ...
            //Push the itinerary.
            for (int i = target; i != source; i = d.at(i).second) {  // replaced previous with d.at(i).second
                if (i == -1 || airport_pos.at(i).empty()) {
                    results.clear();
...
//    i.open("../data/airports_iata.csv");  // WHEN EXECUTING FROM MAIN
    i.open("./data/airports_iata.csv");  // WHEN EXECUTING FROM WEB SERVER
}

void engine::openRoutes(std::ifstream& i) {
//    i.open("../data/routes_final.csv"); // WHEN EXECUTING FROM MAIN
    i.open("./data/routes_final.csv");  // WHEN EXECUTING FROM WEB SERVER
}
